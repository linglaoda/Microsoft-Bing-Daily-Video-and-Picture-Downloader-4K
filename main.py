...
def download(url: str, fname: str,headers: dict): #下载函数(转自网络)
    # 用流stream的方式获取url的数据
...
while i<ips_len:
  ...
  img_url=ips[i][1]['i2160']

  headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.81 Safari/537.36 Edg/104.0.1293.47',
  }

  download(video_url,dirname+'/video.mp4',headers)
  download(img_url,dirname+'/img.img',headers)
